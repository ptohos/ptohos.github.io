...
def abline(x,slope,xp,yp):
    ...
    yvals = [slope*xvals + intercept for xvals in x]    
    return yvals
...
'''
def fun(x):
    return 4*np.sqrt(1-x**2/25)
x = [-10.1 + 0.0001*k for k in range(202000)]
y1 = list(map(fun,x))
y2 =[-y1[k] for k in range(len(y1))]
...
print(slope,x0,y0)
yline = abline(x,slope,xA,yA)      # kataskeyi eytheias
#
plt.plot(x,y1,'b-')
plt.plot(x,y2,'b-')
plt.plot(x,yline,'r--')
