...
def calc_sympy():
    x = sp.Symbol('x')
    func = -x**np.pi + np.pi*x
    return sp.integrate(func,(x,0,np.pi/2))
...
labels=('Ntries','Sampling Method','MC-Theory','Mesi timi', 'MC-Theory', 'Simpy')
print("%10s %5s %10s %10s %10s %10s" % labels)
for ktry in range(ncases):
    ntries = nshots[ktry]
    result2 = mc_mesi_timi(func1,lolim,uplim,ntries)
    result1 = mc_sampling(func1,lolim,uplim,ntries)
    print("%9d %12.6f %13.6f %10.6f %11.6f %11.6f"%(ntries, result1, np.abs(result1-analytiki), result2, np.abs(result2-analytiki),sympyres))
